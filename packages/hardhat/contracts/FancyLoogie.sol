...
contract FancyLoogie is ERC721Enumerable, IERC721Receiver, Ownable {
	...
	function tokenURI(uint256 id) public view override returns (string memory) {
		...
		string memory name = string(
			abi.encodePacked("FancyLoogie #", id.toString())
		);
	...
	function renderTokenById(uint256 id) public view returns (string memory) {
		...
		render = string(
			abi.encodePacked(render, loogies.renderTokenById(loogieById[id]))
		);
	...
	function onERC721Received(
		...
		require(
			...
			"you can only add stuff to a fancy loogie you own."
		);
		require(
			nftContractsAvailables[msg.sender],
			"the loogies can't wear this NFT"
		);
		require(
			nftById[msg.sender][fancyId] == 0,
			"the loogie already has this NFT!"
		);
